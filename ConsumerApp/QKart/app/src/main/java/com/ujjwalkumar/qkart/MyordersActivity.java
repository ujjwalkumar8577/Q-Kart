...
public class MyordersActivity extends AppCompatActivity {
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		imageviewback.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _view) {
				...
				inmo.setClass(getApplicationContext(), HomeActivity.class);
			...
			public void onDataChange(DataSnapshot _dataSnapshot) {
				...
				for(int _repeat13 = 0; _repeat13 < (int)(lmp_orders.size()); _repeat13++) {
					if (lmp_orders.get((int)t).get("custuid").toString().equals(UID)) {
						cmap = new Gson().fromJson(lmp_orders.get((int)t).get("sellermap").toString(), new TypeToken<HashMap<String, Object>>(){}.getType());
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _view, ViewGroup _viewGroup) {
			...
			final ImageView imageviewnav = (ImageView) _v.findViewById(R.id.imageviewnav);
			...
			if (Double.parseDouble(filtered.get((int)_position).get("status").toString()) > 1) {
				imageview2.setImageResource(R.drawable.ic_check_circle_black);
			}
			if (Double.parseDouble(filtered.get((int)_position).get("status").toString()) > 2) {
				imageview3.setImageResource(R.drawable.ic_check_circle_black);
			}
			if (Double.parseDouble(filtered.get((int)_position).get("status").toString()) > 3) {
				imageview4.setImageResource(R.drawable.ic_check_circle_black);
			}
